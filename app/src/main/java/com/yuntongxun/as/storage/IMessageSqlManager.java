...
public class IMessageSqlManager extends AbstractSQLManager {
    ...
    public static void checkContact(String contactid) {
        checkContact(contactid, null, null);
    }

    public static void checkContact(String contactid, String username, String imgUrl) {
        boolean has = true;
        ECContacts c = ContactSqlManager.getContact(contactid);
        if(c==null) {
        ...
        if(!has && (TextUtils.isEmpty(c.getRemark()) || TextUtils.isEmpty(c.getNickname()))){
            // todo 网络请求用户信息
            ToastUtil.showMessage(IMessageSqlManager.class.getSimpleName()+":需要网络请求获取用户基本信息");

            //加入网络请求后去除
            c.setNickname(contactid);
            c.setRemark("drawable://"+ R.drawable.personal_center_default_avatar);
