...
import com.yuntongxun.as.common.view.SettingItem;
...
import com.yuntongxun.as.ui.chatting.base.EmojiconTextView;
import com.yuntongxun.as.ui.contact.ECContacts;
import com.yuntongxun.as.ui.settings.EditConfigureActivity;
	...
	private AdapterView.OnItemClickListener mItemClickListener = new AdapterView.OnItemClickListener() {
		...
		public void onItemClick(AdapterView<?> parent, View view, int position,
			...
			if ("add@yuntongxun.com".equals(item.getVoipAccount())) {
				...
				// todo 跳转到邀请好友页面
				ToastUtil.showMessage("跳转到邀请好友页面");
...
//		}
	...
	public void onClick(View v) {
		...
		} else if (i == R.id.group_showmember_btn) {
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == 0x2a) {
			...
			if (mEditMode == CONFIG_TYPE_GROUP_NAME) {
...
//			changeModifyPermission(changePermission);
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (item.getVoipAccount().equals("add@yuntongxun.com") && position == getCount() - 1) {
				...
				ImageLoader.getInstance().displayImage(item.getRemark(),mViewHolder.mAvatar);
				String creator;
	...
	private void changeModifyPermission(boolean  permission) {
		if (permission && (isManager() || isLocalDiscussion)) {
            mNameItem.setOnClickListener(new OnItemClickListener( CONFIG_TYPE_GROUP_NAME));
            mNoticeItem.setOnClickListener(new OnItemClickListener( CONFIG_TYPE_GROUP_NOTICE));
        } else {
